...
router.get('/getBeneficiaryByUC', isAuthenticated, function (req, res) {
	async.series({
		Beneficiary: function (callback) {
			db.query('SELECT Beneficiary.CNIC, Beneficiary.FirstName, Beneficiary.LastName, \
			Beneficiary.PhonePrimary, Beneficiary.PhoneSecondary, Beneficiary.VHLatitude, \
			Beneficiary.VHLongitude, Beneficiary.BenStatusID, \
			max(DistributedCheques.InstallmentType) as InstallmentType, \
			max(Monitoring.LevelStatus) as LevelStatus \
			FROM Beneficiary \
			LEFT JOIN DistributedCheques on Beneficiary.CNIC = DistributedCheques.CNIC \
			LEFT JOIN Monitoring on Beneficiary.CNIC = Monitoring.CNIC \
			WHERE BenStatusID = 4 AND Beneficiary.UCID = ? \
			GROUP BY Beneficiary.CNIC', [req.query.UCID], callback)
		},
